...
- **`vec_type` now resolves per call from `config.yaml` (`config.yaml > env > default`).** `mnemosyne/core/beam.py` froze `VEC_TYPE` at import from `MNEMOSYNE_VEC_TYPE` alone, so `mnemosyne config set vec_type` required a restart and a `config.yaml` value was ignored entirely. `_resolve_vec_type()` reads on every call with the central precedence (blank counts as unset, an invalid value warns and falls back to `float32` as before); the vector-type probe and the recall fingerprint use the live value so a change cannot serve stale quantized rows, while `VEC_TYPE` remains as an import-time snapshot for backward compatibility.

- **Embedding resolution now honors `config.yaml` (`config.yaml > env > default`).** `mnemosyne/core/embeddings.py` previously read only environment variables, so a Hermes-spawned process with a correct `config.yaml` (`embedding_dim: 1024`) but no `MNEMOSYNE_*` env fell back to 384 and disabled vector search with "Embedding dimension mismatch" on every recall. Model, dimension, API URL/key and the `embeddings_via_api` flag now resolve through the central config with the documented precedence (a stored `.../embeddings` suffix is stripped so both URL conventions agree, and the API key re-resolves lazily so `mnemosyne config set` applies without a restart). The fail-loud contract is unchanged: an unknown model with no explicit dimension — in neither `config.yaml` nor env — still raises at import before any database write. Seeding also stops fragmenting under `HERMES_HOME`: a per-profile path falls back to the shared `~/.hermes/mnemosyne/config.yaml`, inherits its `embedding_*`/`vec_type` values when seeded, and sniffs an existing database's vec dimension (leaving the key unset when no source can vouch for it, so fail-loud still fires).
