...
- **Memory operations now support granular author stamping.** Tool calls can specify `author_id` and `author_type` arguments per-call for precise provenance; batch operations can define a default author for all staged writes; and the consolidation process now persists the author at the row level. Complements #923.
