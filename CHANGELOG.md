...
- **Canonical slots are matched on CJK 2-gram units instead of single characters (#971).** The Hermes provider tokenized spaceless Japanese, Korean and Chinese text one character at a time, and both canonical paths (`_canonical_recall_rows`, `_canonical_prefetch_rows`) required only two distinctive tokens plus a 0.30 coverage floor, so any two characters shared with a slot were enough. Unrelated single-source-of-truth facts therefore scored 0.94-0.98 and were injected into the `## Mnemosyne Context` block of every turn, crowding out precise working and episodic recall. Canonical matching now uses overlapping 2-gram units for CJK runs (`_prefetch_lexical_units`), with function-word units filtered so a short Chinese question such as "什么时候部署？" still matches on its single topical unit. Latin words, the gate structure, the single-token rarity exception and the score formula are unchanged, and `_prefetch_tokens()` still backs core lexical evidence. On synthetic Japanese/Korean/Chinese fixtures, canonical matches for unrelated queries (30, 29 and 2 rows per path) drop to zero on both paths while every true positive survives as the only row returned. The prefetch dedup step signs rows with a dedicated 2-gram unit signature that deliberately ignores the canonical stop-unit settings, so a canonical row and a spaced ordinary result that merely share characters are no longer collapsed into one row while tuning slot matching cannot change how ordinary rows deduplicate (review: coderabbitai). Three boundary cases raised in review are handled inside the same unit builder: the ideographic iteration mark is folded away, so `佐々木` and `佐々野` compare as `佐木` and `佐野` instead of sharing a single-character unit; a 2-gram formed entirely inside configured function-word spans is dropped, so `すること` yields no lexical units at all instead of leaving the bridge unit `るこ`; and the katakana middle dot is a separator, so `猫・犬` yields the same units as `猫、犬` (review: dplush).
