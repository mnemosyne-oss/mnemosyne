...
- **`hermes_memory_provider` disabled memory for a whole session after one transient SQLite lock.** The init-retry fix (#496) landed only in `mnemosyne_hermes`, so the sibling `hermes_memory_provider` copy — the one this repo's own README tells users to symlink into `~/.hermes/plugins/mnemosyne` — kept the fail-once behavior. A `database is locked` raised inside `initialize()` stored `_init_error` permanently; every later `handle_tool_call()` short-circuited on `if not self._beam` and replayed that stale exception as `status: memory_unavailable`, while the database was healthy again seconds later. Observed live 2026-08-07: a session initialized during a write and stayed memory-less for hours; sibling sessions started minutes later initialized fine, and only a restart recovered the affected one. The transient classifier, retry stash, and per-turn `_maybe_retry_init()` calls on `system_prompt_block`, `prefetch`, `sync_turn`, and `handle_tool_call` are now ported to this copy, with the "retried automatically" prompt hint. Non-transient failures (corrupt DB, bad permissions, schema mismatch) keep the fail-once behavior so retries cannot spam the log.
