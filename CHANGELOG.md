...
- **Memory operations now support granular author stamping.** Tool calls can specify `author_id` and `author_type` arguments per-call for precise provenance; batch operations can define a default author for all staged writes; and the consolidation process now persists the author at the row level, inheriting the source attribution only when every source row agrees on the complete `(author_id, author_type)` pair — on any mismatch both fields fall back to the BEAM identity together, so a consolidated summary can never mix halves of two different identities. Complements #923.
