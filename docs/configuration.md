...
| `MNEMOSYNE_EMBEDDING_MAX_CHARS` | `6000` | Caps each **character** (not token) of text sent per input to the API embedding endpoint; the head of the text is kept. Local OpenAI-compatible servers (llama.cpp `llama-server` with `-np > 1`) give each request a context slot of `n_ctx / np` tokens and reject longer inputs with HTTP 400, which aborts the whole `reindex` batch (`reindex_vectors` stops after rebuilding vector tables and `_embed_api` returns `None`). Lower this value for local models with a smaller embedding context window; set it to `0` or a negative value to disable the cap and send full texts. The local fastembed path is not affected. |
